...
| `vpc_id`    | No       | The VPC ID that you want to host the environment. Leave empty to use the default VPC of the region (or the first available VPC when the account has no default VPC). If provided, the VPC must exist and can assign public IP addresses for the EC2 instances.                                                                                                                                                                                                                                                                                                                                                  |
| `subnet_id` | No       | The subnet where you want to create the EC2 instances. Leave empty to use a public subnet (auto-assign public IP) of the resolved VPC. Must be provided together with `vpc_id`.                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| `key_name`  | No       | The AWS key pair that will be assigned to all the EC2 instances. When not provided, the local part of `email` (before `@`) is used. A local key pair at `~/.ssh/<key_name>` and `~/.ssh/<key_name>.pub` is imported into AWS as-is. When it is missing, Terraform generates a new ED25519 key pair on the first apply, imports its public key into AWS and saves the private key to `~/.ssh/<key_name>` (with 0600 permissions); later applies reuse the saved key. If the key pair already exists on AWS but is not in the Terraform state, import it once with `terraform import aws_key_pair.vm <key_name>`. |

> **Note:** when Terraform generates the key pair, keep `~/.ssh/<key_name>` in place. Deleting it makes the next apply generate a new key and replace the AWS key pair, which breaks SSH access to the existing instances.
...
| Field                       | Required | Description                                                                                                                                                                                                                 |
| --------------------------- | -------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `ami_id`                    | No       | EC2 AMI ID for the Ops Manager application servers. When null, `default_ami_id` is used.                                                                                                                                    |
| `version`                   | No       | Ops Manager version, for example `8.0.16`. The download URL is resolved from the Ops Manager release archive: the newest package whose name contains `version`. When null, the newest available package is used.            |
| `tier`                      | No       | EC2 instance type for the Ops Manager application servers.                                                                                                                                                                  |
| `root_size_gb`              | No       | Root volume size in GB for the Ops Manager application servers.                                                                                                                                                             |
| `instance_count`            | No       | Number of Ops Manager application server instances.                                                                                                                                                                         |
| `backup_type`               | No       | Backup store type. Valid options are `s3`, `mongo`, `fileSystem`, and `none`.                                                                                                                                               |
| `appdb.ami_id`              | No       | EC2 AMI ID for the Ops Manager application database. When null, `default_ami_id` is used.                                                                                                                                   |
| `appdb.tier`                | No       | EC2 instance type for the Ops Manager application database.                                                                                                                                                                 |
| `appdb.version`             | No       | MongoDB major and minor version for the Ops Manager application database, for example `8.0`.                                                                                                                                |
| `appdb.root_size_gb`        | No       | Root volume size in GB for the Ops Manager application database.                                                                                                                                                            |
| `backing_db.ami_id`         | No       | EC2 AMI ID for the Ops Manager backing database. When null, `default_ami_id` is used.                                                                                                                                       |
| `backing_db.version`        | No       | MongoDB version for the Ops Manager backing database, for example `8.0.16-ent`. When null, the latest patch of the same `major.minor` as `version` is used, looked up from the MongoDB Enterprise Advanced release archive. |
| `backing_db.tier`           | No       | EC2 instance type for the Ops Manager backing database.                                                                                                                                                                     |
| `backing_db.root_size_gb`   | No       | Root volume size in GB for the Ops Manager backing database.                                                                                                                                                                |
| `backing_db.instance_count` | No       | Number of instances in the Ops Manager backing database replica set.                                                                                                                                                        |
