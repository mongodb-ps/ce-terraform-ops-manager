...
| `key_name`  | No       | The AWS key pair that will be assigned to all the EC2 instances. When not provided, the local part of `email` (before `@`) is used. When the local key pair does not exist at `~/.ssh/<key_name>` and `~/.ssh/<key_name>.pub`, Terraform generates a new one, imports it into AWS and saves the private key to `~/.ssh/<key_name>`. If the key pair already exists on AWS but is not in the Terraform state, import it once with `terraform import aws_key_pair.vm <key_name>`. |
